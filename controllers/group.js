...
function create(req, res, next) {
    ...
    async.waterfall([
        ...
        function(inserted_id, callback) {
            var queryStr = "INSERT INTO `members` SET `user_id` = ?, `group_id` = ?, `hash_val` = ?, `created_at` = ?, `updated_at` = ?";
            var queryVal = [decoded_jwt['uid'], inserted_id, sha512(toString(decoded_jwt['uid']) + "UNIQUE" + toString(inserted_id)).toString('hex'), unixTime(new Date()), unixTime(new Date())];
            app.db_connection.query(queryStr, queryVal, function(err, rows, fields) {
                if(err) {
                    callback(err);
                }
...
function join(req, res, next) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            app.db_connection.query(queryStr, queryVal, function(err, rows, fields) {
                if(err) {
                    ...
                    if(rows[0].check_count != 0) {
                        let custom_err = new Error('Requested UserID already added to group!');
                        custom_err.status = 400;
                        next(custom_err);
        ...
        function(callback) {
            var queryStr = "INSERT INTO `members` SET `user_id` = ?, `group_id` = ?, `hash_val` = ?, `created_at` = ?, `updated_at` = ?";
            var queryVal = [user_id, group_id, sha512(toString(user_id) + "UNIQUE" + toString(group_id)).toString('hex'), unixTime(new Date()), unixTime(new Date())];
            app.db_connection.query(queryStr, queryVal, function(err, rows, fields) {
                if(err) {
                    callback(err);
                }
